...
After you create the both apps, you need to open each and head over to the **Internal Testing** tab and complete **Set up internal testing track**. Basically you need to add some testers, click create a new release and upload your **.aab** file manually. This file can be generated by running **fastlane android build**.

If at this point you already have all the information needed (e.g screenshots, privacy policy and contact details), you can head over to the **Dashboard** and complete the **Set up your app** steps.

Pipelines are configured to deploy android application, automatically, to google play internal sharing, however, due to Google play console limitations, the first upload needs to be done manually.
...
Additionally configure applications for iOS and Android as explained below. In case, anything is not clear follow OneSignal official documentation.
...
- Android app uploaded to Google Play Internal App Sharing for a specific pull request;
...
**The variables above does not need to be configured if your project use the same infrastructure configured on workspace level**.
...
- ANDROID_APPLICATION_ID: Id of android application to be used by Google play: internal sharing or store, depending on environment (only for **android-staging**, **android-preview** and **android-production**);
